...
- **Invoicing**: SmartBill — **live on the paid flows** (v1.2 Phase 2): proforma on every order, fiscal invoice on online payment confirm (IPN); pay-at-location invoices stay manual in the SmartBill UI. Best-effort — a SmartBill failure stamps `smartbill.status='failed'`, never breaks a money flow. PDF links/emails, storno, retry queue, e-Factura still planned. See [documentation/roadmap/v.1.2_smartbill.md](documentation/roadmap/v.1.2_smartbill.md)
