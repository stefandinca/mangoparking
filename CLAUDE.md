...
- **Admin** (`['auth','admin','perm:<section>']`): `/admin`, `/admin/{checkins,transactions,cashbook,refunds,vouchers,website,capacity,pricing,shuttle,users,audit,help}`. **`/admin/audit`** ("Jurnal acțiuni") is the full staff-action history — date range + pagination over `auditLog`, open to every admin-access role. **`/admin/website`** ("Public website", admin-only) is the front-end-content hub: tabs for facility **gallery** + **opening hours** (new) and the **promotions / reviews / legal** editors (consolidated — their routes still exist for deep links but are no longer in the sidebar).
