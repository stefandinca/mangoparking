...
const ACCOUNT_NAV = [
  ...
  { path: '/account/vouchers',     icon: 'vouchers',  labelKey: 'account.vouchers' },
...
/**
 * Returns HTML string for account page layout with sidebar + mobile nav.
 * Call `initAccountNav(pageEl)` after mounting to wire the mobile toggle.
 */
export function accountLayout(activePath, contentHtml) {
...
/**
 * Wire the mobile account nav toggle on a page element.
 */
export function initAccountNav(pageEl) {
  const navToggle = pageEl.querySelector('[data-account-nav-toggle]');
  const navDropdown = pageEl.querySelector('[data-account-nav-dropdown]');
