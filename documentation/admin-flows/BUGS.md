...
**Updated 2026-09-04** — client report on LT-VARZW / LT-HXT69 added #37
(**fixed**: the overstay charge was measured from the scheduled pick-up while
the price is measured from the drop-off, so the board demanded extra days the
pricer would never bill — **21 of 28 long-term overstay charges since June were
above what was owed, 264 lei of it purely from the bug**). Running total:
**17 fixed**, 4 partial, 15 open.
...
| # | Bug | Closed by |
|---|-----|-----------|
| 37 | **Overstay billed from the wrong end of the stay — customers charged for days they had already paid for** | **2026-09-04.** `overstayInfo` counted extra days from `pickupAt + 2h`, but the price is `max(1, ceil((pickup - dropoff - 2h) / 24h))` days measured **from the drop-off**. Because `days` is rounded up, the paid-for window runs to `dropoff + 2h + days × 24h`, which is normally **later** than the booked pick-up — so between those two instants the board demanded a day the pricer would never bill. LT-VARZW / LT-HXT69 (6d12h45m sold as 7 days) opened an **11h15m** false window; both cars left inside it and were each quoted **20 lei** they did not owe. Fixed by anchoring on the drop-off in `overstayFreeUntilMs` (`src/utils/bookingTime.js`), with the pick-up anchor kept as a fallback for the 4 of 533 long-term rows that carry no parseable `dropoffAt`. Regression tests sweep hour-by-hour and assert `daysLate` never exceeds what the pricer would bill (`tests/bookingTime.test.mjs`). **Blast radius: 21 of 28 long-term overstay charges since 2026-06 exceeded what was owed — 20 of them should have been zero, 264 lei total** (LT-JJR7Z, LT-UX5S2, LT-2ZVLN, LT-KMR29, LT-SQ6HM, LT-FKWKE, LT-V6F34 at 1 leu each — agents typing a token amount to clear the gate — then LT-2NJLE 17, LT-6YLUU/5VVK9/HJUQA/SVUUZ 18, LT-U28E2 19, LT-XLWMH/9PYRB 20, LT-3RYT4 21, LT-S4X5Z/WMCZR/MAU6A/3WZZV 22). LT-MCJTR (177 lei vs 22 due) is **not** this bug — the dialog lets the agent overwrite the figure and they did. Re-run the audit with the drop-off-anchored rule over `bookings` where `latePrice > 0`. **The board still shows a car past its pick-up as overdue** — that stayed, it is an operational signal, not a money one. |
| 36b | **A skipped overstay left no trace at all** | **2026-09-04.** Found with #37: the "check out anyway" override moves no money and wrote nothing, so an overstay the board said was owed could be collected by hand (or waived) with no record anywhere — which is why LT-VARZW / LT-HXT69 needed a Firestore dig to explain. `checkOutBooking(id, { waivedOverstay })` now stamps `overstayWaivedAmount` / `overstayWaivedAt` on the booking and carries the figure into the `booking_checkout` audit row, which `describeAction` renders as "depășire de N lei neîncasată" / "N lei overstay not collected". |
| 35 | **POS-card fiscal invoices never issued — the 2026-08-05 fix was inert** | **2026-08-21.** `adminMarkOrderPaid` gained a `smartbillIssueSafe` call for card collections (decision 1b) but was declared `onCall({ region, cors: true })` with **no `secrets:` array**, so `SMARTBILL_CIF` resolved empty and every desk card payment logged `SmartBill not configured: SMARTBILL_CIF secret is empty` and stamped `smartbill.status='failed'`. Best-effort issuance meant it failed silently — the money flow completed, the document didn't. Found while investigating LT-8DVK5. Fixed by binding `SMARTBILL_SECRETS` on the callable (`index.js:1909`); **needs a functions redeploy to take effect**. **Blast radius: 3 orders, 544 lei** — LT-REDYW, LT-U8ZUE, LT-MAU6A — **backfilled 2026-08-21** as Mango 0195/0196/0197 via `scripts/backfill-smartbill-invoices.mjs`. Deployed the same day; nothing since 2026-08-05 is left uninvoiced. To re-audit, read the **booking's** `smartbill` block (`paidBy == 'admin-card'`, no `smartbill.invoice`): the pre-2026-08-21 backfill stamped only the booking, so the same query over `pendingOrders` reports false positives for LT-EC8LV / LT-FKWKE / LT-KD5VA / LT-6ACKD (invoiced Mango 0115–0118). The script now mirrors onto the order too. The 5 remaining uninvoiced card rows all predate SmartBill going live on 2026-07-16 and are the known "rămase de rezolvat" bucket in [../bugs/raport-facturi-card.md](../bugs/raport-facturi-card.md). |
