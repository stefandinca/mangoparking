...
> **DROPPED — client decision 2026-07-17.** Documents are not surfaced in the
> app: customers don't need them, staff check SmartBill directly. An
> `invoicePdf` HTTP proxy + UI links shipped briefly and were reverted the
> same day (implementation preserved in git history, commit `7d87fa4`).
> Relevant finding if ever revived: plain SmartBill invoices have NO public
> URL (verified) — only `/invoice/reverse` returns a tokenized view link.
...
> **DROPPED — client decision 2026-07-17** (same as Phase 5: documents are not
> sent to customers). The `invoiceUrl`/`proformaUrl` email params shipped
> briefly and were removed; no Brevo template changes are needed.
